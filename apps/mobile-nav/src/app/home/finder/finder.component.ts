    ...
    <umn-page #page 
      ...
      <ion-header *ngIf="pageHelper?.isMobile || type =='full_screen'">
        <ion-toolbar>
          ...
          <ion-title>{{page?.path}} {{type}}</ion-title>
        </ion-toolbar>
      </ion-header>
      <ion-content>
        ...
        <ion-row *ngIf="pageHelper?.isMobile || type == 'full_screen'">
          <ion-col class="iconCol" size="4" *ngFor="let entry of directory" [path]="entry.path">
            <ion-icon [color]="entry.iconColor"
...
export class FinderPageComponent implements AfterViewInit {
  ...
  private async onRouteChange() {
    if (this.id && this.explorerController?.path) {
      // console.log(window.location.href.substr(window.location.href.indexOf(FinderPageComponent.foot)), params["id"])
